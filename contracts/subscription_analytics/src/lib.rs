...
impl SubscriptionAnalytics {
    pub fn initialize(env: Env, admin: Address, sub_contract: Address) {
        ...
        env.storage().persistent().set(&DataKey::SubContract, &sub_contract);
        env.storage().persistent().set(&DataKey::TotalMRR, &0i128);
        env.storage().persistent().set(&DataKey::ActiveSubscribers, &0u32);
    }

    pub fn record_subscription_event(env: Env, event_type: SubEvent, _plan_id: Symbol, amount: i128) {
        let sub_contract: Address = env.storage().persistent().get(&DataKey::SubContract).expect("Not initialized");
        sub_contract.require_auth();

        let (year, month) = get_month_year(env.ledger().timestamp());
        let metrics_key = DataKey::Metrics(year, month);
        let mut metrics: MonthlyMetrics = env.storage().persistent().get(&metrics_key).unwrap_or_default();
        let mut active_mrr: i128 = env.storage().persistent().get(&DataKey::TotalMRR).unwrap_or(0);
        let mut active_subs: u32 = env.storage().persistent().get(&DataKey::ActiveSubscribers).unwrap_or(0);
        ...
        env.storage().persistent().set(&DataKey::TotalMRR, &active_mrr);
        env.storage().persistent().set(&DataKey::ActiveSubscribers, &active_subs);

        env.events().publish(
            (symbol_short!("metrics"), symbol_short!("updated"), year, month),
            (event_type, amount),
        );
    }

    pub fn get_mrr(env: Env, month: u32, year: u32) -> i128 {
        let metrics: MonthlyMetrics = env.storage().persistent()
    ...
    pub fn get_active_subscribers(env: Env) -> u32 {
        env.storage().persistent().get(&DataKey::ActiveSubscribers).unwrap_or(0)
    }
...
mod test {
    ...
    fn test_churn_rate() {
        ...
        let plan_id = symbol_short!("PLAN1");
        for _ in 0..10 { client.record_subscription_event(&SubEvent::NewSubscription, &plan_id, &100i128); }
